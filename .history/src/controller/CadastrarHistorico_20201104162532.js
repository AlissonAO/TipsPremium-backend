...
module.exports = {
  async obterProximaCorrida(req, res) {
    ...
    const query = {
      DataCorrida: '04-11-2020',
    };
    await client.connect();
    const database = client.db('premiumTips').collection('historicoCalculado');

    const resultado = await database.find(query).forEach(function (item) {
      if (typeof item.DataCorrida == 'string' && item.DataCorrida != null) {
        console.log(item.DataCorrida);
        // item.DataCorrida = new Date(item.DataCorrida);
        // database.updateOne(item);
      }
    });
